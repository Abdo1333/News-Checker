...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \fill[primary!25, opacity=0.2]
    (current page.north west) -- ++(14,0) -- ++(0,-9) -- cycle;
  \fill[accent!30, opacity=0.12]
    (current page.south east) -- ++(-13,0) -- ++(0,8) -- cycle;
  \foreach \x/\y/\r in {2/25/2.5, 19/4/3.5, 10/14/1.2}{
    \fill[white, opacity=0.04] (\x,\y) circle (\r);
  }
\end{tikzpicture}
...
% Logo
\begin{tikzpicture}
  \node[draw=primary, fill=darkbg, rounded corners=12pt,
        inner sep=18pt, line width=2.5pt,
        ] {
    \begin{tikzpicture}[scale=1.2]
      \draw[primary, line width=2.5pt, rounded corners=3pt]
        (0,0) rectangle (1.2,1.5);
      \foreach \y in {0.3,0.6,0.9,1.2}{
        \draw[accent, line width=1.5pt] (0.15,\y) -- (1.05,\y);
      }
    \end{tikzpicture}
  };
\end{tikzpicture}
...
\vspace{0.8cm}
{\fontsize{42}{50}\bfseries\color{white} NewsChecker}

\vspace{0.2cm}
{\Large\color{accent}\itshape DApp --- Vérification Décentralisée}

\vspace{0.9cm}
\begin{tikzpicture}
  \node[fill=primary!35, rounded corners=5pt,
        inner xsep=16pt, inner ysep=8pt]{
    \color{white}\large\bfseries
    Vérification des Nouvelles sur la Blockchain Tezos
  };
\end{tikzpicture}

\vspace{1.5cm}
{\color{white!60}\large\rule{9cm}{0.4pt}}

\vspace{1cm}
\begin{tabular}{ccc}
  \begin{tikzpicture}
    \node[draw=accent!60, fill=accent!12, rounded corners=5pt,
          inner sep=8pt, text=white, align=center, minimum width=3.8cm]{
  ...
  &\quad&
  \begin{tikzpicture}
    \node[draw=success!60, fill=success!12, rounded corners=5pt,
          inner sep=8pt, text=white, align=center, minimum width=3.8cm]{
      \textbf{Frontend DApp}\\
      {\small React 19 + Taquito}
    };
  \end{tikzpicture}
\end{tabular}

\vspace{2cm}
{\color{white!70}\large
  \textbf{Auteurs :} Maram \;\textbullet\; Abderrazzak \;\textbullet\; Ayoub \;\textbullet\; Asmaa}

\vspace{0.5cm}
{\color{white!60}
  \textbf{Réseau :} Tezos Ghostnet (Testnet) \quad
  \textbf{Version :} 1.0.0 \quad
  \textbf{Date :} Mars 2026}

\vspace{0.8cm}
{\color{white!40}\small\itshape
  Système de vérification d'informations basé sur la réputation et le consensus décentralisé\\
  Résistant aux attaques Sybil --- Transparent --- Immuable}
